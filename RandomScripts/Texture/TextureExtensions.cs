...
namespace TextureExtensions
{
    ...
    public static class Shapes
    {
        ...
        public static void Rotate(this Texture2D mask, float angle)
        {                                    
            Color32[] pix1 = mask.GetPixels32();
            Color32[] pix2 = mask.GetPixels32();
            ...
            Color32[] pix3 = rotateSquare(pix2, (Math.PI / 180 * (double)angle), mask);
            ...
            for (int j = 0; j < H; j++)
            {
                for (var i = 0; i < W; i++)
                {
                    ...
                    pix1[mask.width / 2 - W / 2 + x + i + mask.width * (mask.height / 2 - H / 2 + j + y)] = pix3[i + j * W];
                }
            }
            
            mask.SetPixels32(pix1);                                    
        }

        private static Color32[] rotateSquare(Color32[] arr, double phi, Texture2D mask)
        {
            int x;
            int y;
            int i;
        ...
        public struct Pixel
        {
            public short x;
            public short y;
            public Pixel(short aX, short aY) { x = aX; y = aY; }
            public Pixel(int aX, int aY) : this((short)aX, (short)aY) { }
        }
        ...
        public static void FloodFillArea(this Texture2D aTex, int aX, int aY, Color aFillColor)
        {
            ...
            Queue<Pixel> nodes = new Queue<Pixel>();
            nodes.Enqueue(new Pixel(aX, aY));
            while (nodes.Count > 0)
            {
                Pixel current = nodes.Dequeue();
                for (int i = current.x; i < w; i++)
                {
                    ...
                    if (current.y + 1 < h)
                    {
                        ...
                        if (C == refCol && C != aFillColor)
                            nodes.Enqueue(new Pixel(i, current.y + 1));
                    }
                    if (current.y - 1 >= 0)
                    {
                        C = colors[i + current.y * w - w];
